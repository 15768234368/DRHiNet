...
class Visualizer:
    ...
    def update_losses(self, losses, run_name, bitwise_err, psnr_c, *args):
        print('\r', '    '*20, end='')
        line = '\r%.3i' % (self.counter)
        for l in losses:
            line += '\t\t%.4f' % (l)
        line += ' ' * 20 + run_name + ' ' * 20 + 'bitwise_err:' + str(bitwise_err) + ' ' * 20 + 'psnr_c' + str(psnr_c)
        print(line)
        self.counter += 1
...
def show_loss(losses, run_name, bitwise_err, psnr_c, logscale=False):
    visualizer.update_losses(losses, run_name, bitwise_err, psnr_c)
